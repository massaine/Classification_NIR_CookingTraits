...
###load data
```{r,eval=FALSE}
load(file=here::here("output","dadosBUCHI_Mas_PT.RData"))
dados1 <- dados2[[7]]
save(dados1,file=here::here("output","dados1_BUCHI_Mas.RData"))
```
...
trait <- Cooking_traits[1]
  ...
  Ns <- round(table(training[,trait])[1]) - (nrow(OvSam)*16) 
  sample1 <- sample(x = c(1:nrow(OvSam)), size = Ns, replace = TRUE)
  x <- do.call(rbind, replicate(15, OvSam, simplify=FALSE))
 ...
 save(testing,modelFit1, 
      file = here::here("output", "KNN_BUCHI_Model_Cook15.RData"))
...
trait <- Cooking_traits[2]
  ...
  OvSam <- subset(training,training[,trait]=="Yes")
  Ns <- round(table(training[,trait])[1]) - (nrow(OvSam)*7) 
  sample1 <- sample(x = c(1:nrow(OvSam)), size = Ns, replace = TRUE)
  x <- do.call(rbind, replicate(6, OvSam, simplify=FALSE))
  training1 <- rbind(training,x, OvSam[sample1,] )# ,OvSam[sample1,])
  ...
  save(testing,modelFit1, 
      file = here::here("output", "KNN_BUCHI_Model_Cook20.RData"))
...
save(testing,modelFit1, 
      file = here::here("output", "KNN_BUCHI_Model_Cook25.RData"))
...
```{r}
save(testing,modelFit1, 
     file = here::here("output", "KNN_BUCHI_Model_Binarias.RData"))
```
